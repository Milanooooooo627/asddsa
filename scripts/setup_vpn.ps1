# Proton HWID Spoofer - OpenVPN Setup with NordVPN
...
$helperPath = $null
if ($scriptDir) {
    $helperPath = Join-Path $scriptDir "Proton-Helpers.ps1"
} else {
    $helperPath = Join-Path (Get-Location) "Proton-Helpers.ps1"
}
...
function Get-NordVpnCountries {
    try {
        ...
        $webClient.Headers.Add("User-Agent", "Proton HWID Spoofer")
...
function Create-CustomConfig {
    ...
    # --- BEGIN Proton Fix: Ensure CA certificate and TLS-auth key are always embedded ---
...
# Main script
Clear-Host
Write-Host "=== Proton HWID Spoofer - VPN Setup ===" -ForegroundColor Cyan
...
if (Check-OpenVpnInstalled) {
    ...
    if ($openVpnInstaller -and (Test-Path $openVpnInstaller)) {
        ...
        $searchPaths = @(
            "C:\Users\hJlsa90213Aa1\Downloads\Proton Final 1\Proton Final\tools\OpenVPN-2.6.14-I001-amd64.msi",
            "C:\Users\hJlsa90213Aa1\Downloads\Proton Final\tools\OpenVPN-2.6.14-I001-amd64.msi",
            "C:\Users\hJlsa90213Aa1\Desktop\Proton Final\tools\OpenVPN-2.6.14-I001-amd64.msi"
        )
